...
import { BludclartPlayer } from "@/components/player/BludclartPlayer";
...
export function RealPlayerView() {
  ...
  // Use Bludclart as the primary player if we have a tmdbId
  const isMovie = !params.season;
  
  if (params.media && usePlayerStore.getState().sourceId === 'bludclart') {
    return (
      <PlayerPart backUrl={useLastNonPlayerLink()} onMetaChange={() => {}}>
         <BludclartPlayer 
            tmdbId={params.media} 
            type={isMovie ? 'movie' : 'tv'} 
            season={params.season ? parseInt(params.season) : undefined}
            episode={params.episode ? parseInt(params.episode) : undefined}
         />
      </PlayerPart>
    );
  }

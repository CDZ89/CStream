...
export const UniversalPlayer = ({
  ...
  // isPlayerReady: start as TRUE so iframe renders immediately without requiring a click
  ...
  const handleSourceChange = useCallback((sourceId: PlayerSource) => {
    ...
    if (!propSetCurrentSource) {
      try { localStorage.setItem("cstream_preferred_player", sourceId); } catch { }
    }
    ...
    autoSwitchAttemptRef.current = 0; // reset auto-switch tries on manual change
    ...
    <div className={cn("flex flex-col w-full gap-4", className)}>
      <div className="flex-1 flex flex-col min-w-0 shadow-2xl rounded-2xl overflow-hidden ring-1 ring-white/10">
        ...
        >
          ...
          {/* Iframe always rendered immediately — no gating */}
          <iframe
            ref={iframeRef}
            key={iframeKey}
            src={videoUrl}
